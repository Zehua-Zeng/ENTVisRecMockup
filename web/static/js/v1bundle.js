...
function onClickEvent(e) {
    ...
    for (wrapper of wrappers) {
        let item = `v1_${wrapper.classList.item(1).split("_wrapper")[0]}`;
        if (
...
function toggleBookMark(e) {
    ...
    if (btn.getAttribute("added") == "true") {
        ...
        window.localStorage.removeItem(`v1_${str.split("_wrapper")[0]}`);
        ...
        window.localStorage.setItem(`v1_${splittedStr}`, queryMap[splittedStr]);
 ...
 * if f is not null, then we are generating query with extra field f (related views, has been disabled for this version).
