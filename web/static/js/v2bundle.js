      ...
      (function (global, factory) {
        ...
        ***************************************************************************** */
        ...
        function extendConfig(opt) {
          return Object.assign({}, DEFAULT_QUERY_CONFIG, opt, {
            enum: extendEnumIndex(opt.enum)
          });
        }

        function extendEnumIndex(enumIndex) {
          const enumOpt = Object.assign({}, DEFAULT_ENUM_INDEX, enumIndex, {
        ...
        function getVlStack(specQ) {
          ...
          const encoding = toEncoding(specQ.encodings, {
            schema: null,
            wildcardMode: 'null'
          });
          const mark = specQ.mark;
          return stack(mark, encoding, undefined, {
            disallowNonLinearStack: true
          });
        }
        /**
        ...
        function hasRequiredStackProperties(specQ) {
          ...
          for (const encQ of encodings) {
            if (objectContainsWildcard(encQ, {
                exclude: exclude
              })) {
              return false;
            }
              ...
              .reduce((encQs, encQ) => {
                ...
                if (!isDisabledAutoCountQuery(encQ)) {
                  ...
                  if (!!stack && encQ.channel === stack.fieldChannel) {
                    str = encoding(Object.assign({}, encQ, {
                      stack: stack.offset
                    }), include, replace);
                  } else {
            ...
            if (fieldDefShorthand[0] === '?') {
              ...
              for (let encodingProperty in fnEnumIndex) {
                if (util_1(fnEnumIndex[encodingProperty])) {
                  fieldQ[encodingProperty] = {
                    enum: fnEnumIndex[encodingProperty]
                  };
                } else {
        ...
        function toFieldDef(encQ, params = {}) {
          const {
            props = DEFAULT_PROPS, schema, wildcardMode = 'skip'
          } = params;
          if (isFieldQuery(encQ)) {
            const fieldDef = {};
            for (const prop of props) {
              ...
              if (prop === Property.SCALE && schema && encQ.type === ORDINAL) {
                ...
                const {
                  ordinalDomain
                } = schema.fieldSchema(encQ.field);
                if (scale !== null && ordinalDomain) {
                  fieldDef[Property.SCALE] = Object.assign({
        ...
        function isContinuous$1(encQ) {
          if (isFieldQuery(encQ)) {
            return isContinuous(toFieldDef(encQ, {
              props: ['bin', 'timeUnit', 'field', 'type']
            }));
          }
        ...
        function isDimension(encQ) {
          if (isFieldQuery(encQ)) {
            const fieldDef = toFieldDef(encQ, {
              props: ['bin', 'timeUnit', 'type']
            });
        ...
        function build(data, opt = {}, tableSchema = {
          ...
          const derivedTableSchema = Object.assign({}, tableSchema, {
            fields: fieldSchemas
          });
          ...
          cardinality(fieldQ, augmentTimeUnitDomain = true, excludeInvalid = false) {
            ...
            if (fieldQ.aggregate || (isAutoCountQuery(fieldQ) && fieldQ.autoCount)) {
              ...
              if (!timeStats || !timeStats[unit]) {
                timeStats = Object.assign({}, timeStats, {
                  [unit]: timeSummary(fieldQ.timeUnit, fieldSchema.stats)
                });
              }
              if (excludeInvalid) {
          ...
          timeUnitHasVariation(fieldQ) {
            ...
            for (let timeUnitPart of TIMEUNIT_PARTS) {
              if (containsTimeUnit(fullTimeUnit, timeUnitPart)) {
                ...
                const singleUnitEncQ = util_5({}, fieldQ, {
        ...
        const FIELD_CONSTRAINTS = [{
          ...
          },
          {
            ...
            satisfy: (fieldQ, schema, encWildcardIndex, opt) => {
              ...
              const {
                compatible
              } = channelCompatibility(fieldDef, fieldQ.channel);
              if (compatible) {
                return true;
              } else {
        ...
        const REPLACE_XY_CHANNELS = {
          x: 'xy',
          y: 'xy'
        };
        const REPLACE_FACET_CHANNELS = {
          row: 'facet',
          column: 'facet'
        };
        ...
        class SpecQueryModel {
              ...
              encQ[prop] = util_5({}, encQ[prop], // copy all existing properties
                {
                  enum: undefined,
                  name: undefined
                } // except name and values to it no longer an wildcard
              );
            } else {
          ...
          toSpec(data) {
            ...
            spec.encoding = toEncoding(this.specQuery.encodings, {
              schema: this._schema,
              wildcardMode: 'null'
            });
            if (this._spec.width) {
        ...
        function recommend(q, schema, config) {
          ...
          q = Object.assign({}, normalize$1(q), {
            config: Object.assign({}, DEFAULT_QUERY_CONFIG, config, q.config)
          });
          // 2. Generate
    ...
    function readFields() {
      ...
      for (box of allCheckboxes) {
        if (box.checked) {
          ...
          if (s == "Rotten_Tomatoes") checkedNames[i] = "Rotten_Tomatoes_Rating";
          else checkedNames[i] = s;
          i++;
        }
      }
